...
#KafkaMirrorMakerSpec: ({
	...
	consumer: {
		...
		// The MirrorMaker consumer config. Properties with the following
		// prefixes cannot be set: ssl., bootstrap.servers, group.id,
		...
		// Specifies the offset auto-commit interval in ms. Default value
		// is 60000.
		offsetCommitInterval?: int
		tls?: {
	...
	// The container image used for Kafka MirrorMaker pods. If no
	// image name is explicitly specified, it is determined based on
	// the `spec.version` configuration. The image names are
	...
	// expressions. Mirroring two topics named A and B is achieved by
	// using the expression `A\|B`. Or, as a special case, you can
	// mirror all topics using the regular expression `*`. You can
	// also specify multiple regular expressions separated by commas.
	include?: string
	...
	producer: {
		// Flag to set the MirrorMaker to exit on a failed send. Default
		// value is `true`.
		...
		// The MirrorMaker producer config. Properties with the following
		// prefixes cannot be set: ssl., bootstrap.servers, sasl.,
		// security., interceptor.classes (with the exception of:
		// ssl.endpoint.identification.algorithm, ssl.cipher.suites,
		// ssl.protocol, ssl.enabled.protocols).
		config?: {
			...
	...
	// The Kafka MirrorMaker version. Defaults to the latest version.
	// Consult the documentation to understand the process required
